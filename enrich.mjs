...
async function scrapeWikiPage(sourceLabel, url) {
  ...
  for (let ti = 0; ti < tableContents.length; ti++) {
    ...
    // Always log every table's headers so we can diagnose filter misses
    ...
    console.log(`  Table ${ti+1} (${parsed.rows.length} rows) [${passes ? '✓' : '✗ skip'}]: ${parsed.headers.map(h => `"${h.slice(0,25)}"`).join(' | ')}`);
    if (!passes) continue;
    ...
    // Print column mapping for passing tables
    ...
    // Print first 2 parsed entries per table so we can verify column mapping
    const sample = results.slice(-Math.min(2, results.length));
    for (const e of sample) {
      console.log(`    sample: series="${e.series}" vol=${e.vol} issues="${(e.rawIssues||'').slice(0,35)}" isbn="${e.isbn}"`);
    }
  }
